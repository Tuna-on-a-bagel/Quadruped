...
class Kinematics:
    ...
    def forwardK(self, angles=None, verbos=False):
        ...
        for idx, leg in enumerate(self.legs):
            ...
            if angles is None: [theta0, theta1, theta2] = leg.jointPos[:, 0]
    ...
    def inverseK(self, leg, footPos, maxIter=50, tol=1e-6, verbose=False, logging=False):
        ...
        '''Args:
                ...
                verbose:    if true will print out solution
                logging:    if true will print out list of iteration history
        ...
        nearestSol = None
        nearestDist = 1000      #arbitrary
        for sol in IK_sols:
            dist = np.linalg.norm(sol - footPos)        #float
            if dist < nearestDist:
                nearestSol = sol[3:, 1].flatten()       # 1x3 array??? wanting this to be joint angles
                nearestDist = dist                      #float

        #TODO: make this a tensor, verify
        x_ = torch.tensor(nearestSol, requires_grad=True)      #initial guess
...
if __name__ == '__main__':
    ...
    K = Kinematics()
    K.setConfig('/home/tuna/Documents/Quad/RKD/roboConf.json')
